...
# Force CPU for data construction to avoid CUDA init in forked subprocesses
...
def get_a_new2(ins_name):
    ...
    for e in obj:
        ...
        if v != 0:
            ...
            # Use the true objective coefficient as the edge weight for the objective row
            values_spr.append(float(v))
        v_nodes[v_indx][0] = v
    ...
    for cind, c in enumerate(cons):
        ...
        for k in coeff:
            ...
            if coeff[k] != 0:
                ...
                indices_spr[1].append(v_indx)
                # Use the true constraint coefficient as the edge weight
                values_spr.append(float(coeff[k]))
            v_nodes[v_indx][2] += 1
            v_nodes[v_indx][1] += coeff[k] / lcons
            v_nodes[v_indx][3] = max(v_nodes[v_indx][3], coeff[k])
            v_nodes[v_indx][4] = min(v_nodes[v_indx][4], coeff[k])
    ...
    # Build sparse matrix with float32 values to match downstream model expectations
    A = torch.sparse_coo_tensor(indices_spr, values_spr, (ncons + 1, nvars), dtype=torch.float32).to(device)
    clip_max = [20000, 1, torch.max(v_nodes, 0)[0][2].item()]
    clip_min = [0, -1, 0]
